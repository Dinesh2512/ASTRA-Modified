...
#define array_replace(type, a, i, datum) do {\
  -(a)->index != sizeof(type) ? array_abort((a),4) : 0;\
  int k = (i);\
  int j;\
  array_t *(b) = ALLOC(array_t*, 1);\
  (b)->num = (a)->num;\
  (b)->n_size = (a)->num;\
  (b)->obj_size = (a)->obj_size;\
  (b)->index = -(a)->obj_size;\
  (b)->space =  ALLOC(char, (b)->n_size * (b)->obj_size);\
  for((j)=(0);(j)<(b)->num;++(j))\
    (j)==(k) ? array_insert(type, b, j, datum) : array_insert(type, b, j, array_fetch(type, a, j));\
  FREE((a));\
  (a)=(b);\
} while (0)
